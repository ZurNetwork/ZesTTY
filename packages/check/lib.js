 ...
 *
 * Match the MODULE, never the specifier list. The list is not fixed and
 * never was: 0.5.0 range arms add `__ztsInRange`, so a range twin imports
 * `{ __ztsAbsurd, __ztsInRange }` — or just `{ __ztsInRange }` when every
 * match ends in `_`. A specifier-exact pattern missed those, the staleness
 * comparison then re-compiled in INLINE mode, and every committed range
 * twin read stale forever (reproduced end-to-end). Any future preamble
 * export is covered by construction.
 */
const PREAMBLE_IMPORT_RE = /^import \{[^}]*\} from "@zestty\/core";$/m;
